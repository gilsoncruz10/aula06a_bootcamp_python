...
while True:
    ...
    with placeholder_kpi.container():
        ...
        col1.metric(
            "Valor Atual (High)", f"{current_value:.2f}"
        )  # Agora o valor é um float
        col2.metric("Total de Registros", total_records)

    # Verificar limites e alarmar
    if (
        current_value < 5.0 or current_value > 6.0
    ):  # Exemplo de limites para o valor 'high'
    ...
    base_chart = alt.Chart(recent_data).encode(
        x=alt.X("time:T", title="Tempo"), tooltip=["time:T", "high:Q"]
    )
    ...
    lower_bound = (
        alt.Chart(pd.DataFrame({"y": [5.0]})).mark_rule(color="red").encode(y="y:Q")
    )

    # Linha de corte superior
    upper_bound = (
        alt.Chart(pd.DataFrame({"y": [6.0]})).mark_rule(color="red").encode(y="y:Q")
    )

    combined_chart = (realtime_line + lower_bound + upper_bound).properties(
        title="Gráfico em Tempo Real de Cotação (High)", width=800, height=400
    )
